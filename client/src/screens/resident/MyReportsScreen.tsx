...
export const MyPickupsScreen: React.FC = () => {
  const { user } = useAuth();
  ...
  const [statusFilter, setStatusFilter] = useState<
    RequestStatus | typeof ALL_STATUS
  >(ALL_STATUS);

  const loadReports = async () => {
    if (!user) return;
    try {
      const pickupData = await pickupService.getMyPickups();

      const normalizedPickups: WasteRequest[] = pickupData.map(
        (p) =>
          ({
            id: p.id,
            residentId: user.id,
            status: (p.status.charAt(0).toUpperCase() +
              p.status.slice(1)) as RequestStatus,
            type: p.type as any,
            street: p.address,
            requestedDate: (p as any).createdAt || new Date().toISOString(),
            severity: "Medium",
            locationId: "",
            cost: p.extraFee,
            notes: p.notes,
  ...
  const filteredReports = reports.filter((r) => {
    ...
    const matchesStatus =
      statusFilter === ALL_STATUS || r.status === statusFilter;
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate("PickupDetails", {
            reportId: item.id,
            isSpecialPickup: (item as any).isSpecialPickup,
          })
        }
      >
        <AppCard
