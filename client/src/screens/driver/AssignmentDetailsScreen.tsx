...
export const AssignmentDetailsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      try {
        ...
        // Fetch real-time weather & traffic
  ...
  const isActive =
    assignment.status === "InProgress" || assignment.status === "Paused";
    ...
    <ScreenContainer scrollable>
      ...
      <View style={styles.heroRow}>
        <AppCard style={styles.heroCard} padded>
          <View style={styles.heroIcon}>
            <Map color={theme.colors.primary} size={20} />
          </View>
      ...
      <View style={styles.conditionsRow}>
        <AppCard style={styles.conditionBox}>
          <AlertTriangle
            color={
              (traffic?.condition || assignment.traffic.condition) === "Heavy"
          ...
          <View>
            <AppText variant="bodySmall" weight="600">
              {weather
                ...
                : `${assignment.weather.temperature} - ${assignment.weather.condition}`}
            </AppText>
            {(weather?.warning || assignment.weather.warning) && (
      ...
      <View style={styles.footer}>
        {!isCompleted ? (
          ...
          <AppButton
            ...
            onPress={() =>
              navigation.navigate("RouteSummary", { routeId: assignment.id })
            }
            variant="outline"
          />
        )}
